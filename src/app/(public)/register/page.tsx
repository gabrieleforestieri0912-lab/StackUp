...
const PASSWORD_RULES = [
  { label: 'Min. 6 caratteri', test: (v: string) => v.length >= 6 },
  { label: 'Una lettera', test: (v: string) => /[a-zA-Z]/.test(v) },
...
const Register: React.FC = () => {
  ...
  const match = confirmPw ? password === confirmPw : null;
    ...
    <main className="min-h-screen flex items-center justify-center px-4 sm:px-6 py-6 bg-black relative overflow-hidden">
      ...
      >
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-jakarta font-extrabold text-zinc-100">Unisciti a noi.</h1>
          <p className="text-sm text-zinc-400 mt-1 font-medium">Inizia il tuo viaggio verso l&apos;eccellenza tecnica.</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-[10px] font-mono font-black uppercase tracking-widest text-zinc-400 mb-1.5">Nome Completo</label>
              <div className="relative group">
                <User className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-500 group-focus-within:text-orange-400 transition-colors" size={16} />
                <input
                  type="text"
                  required
            ...
            <div>
              <label className="block text-[10px] font-mono font-black uppercase tracking-widest text-zinc-400 mb-1.5">Password</label>
              <div className="relative group">
              ...
              {password && (
                <div className="mt-2 space-y-1.5">
                  <div className="flex gap-1">
                    {[0, 1, 2].map((i) => (
                      <div key={i} className={`h-1 flex-1 rounded-full transition-all ${strength > i ? colors[i] : 'bg-zinc-800'}`} />
                    ))}
                  </div>
                  <div className="flex flex-wrap gap-x-4 gap-y-1">
                    {PASSWORD_RULES.map((rule) => {
                      ...
                      return (
                        <span key={rule.label} className={`text-[10px] font-bold font-mono flex items-center gap-1 ${ok ? 'text-emerald-400' : 'text-zinc-600'}`}>
            ...
            <div>
              ...
              {confirmPw && (
                <p className={`mt-1 text-[10px] font-bold font-mono flex items-center gap-1 ${match ? 'text-emerald-400' : 'text-red-400'}`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${match ? 'bg-emerald-400' : 'bg-red-400'}`} />
                  {match ? 'Corrispondono' : 'Non coincidono'}
                </p>
              )}
            </div>
            ...
            <label className="flex items-start gap-3 cursor-pointer group">
              <input
                ...
                className="mt-0.5 w-4 h-4 bg-black border border-zinc-700 rounded-md accent-orange-500 cursor-pointer"
              />
              <span className="text-xs text-zinc-400 group-hover:text-zinc-300 transition-colors leading-relaxed">
                Accetto i{' '}
                <Link href="/terms" className="text-orange-400 hover:text-orange-300 font-bold">Termini e Condizioni</Link>{' '}
                e la{' '}
                <Link href="/privacy" className="text-orange-400 hover:text-orange-300 font-bold">Privacy Policy</Link>
              </span>
            </label>
          ...
          <p className="mt-6 text-center text-xs text-zinc-500 font-medium">
            Hai già un account?{' '}
            <Link href="/login" className="text-orange-400 hover:text-orange-300 font-bold transition-colors">Accedi</Link>
          </p>
      </motion.div>
    </main>
  );
